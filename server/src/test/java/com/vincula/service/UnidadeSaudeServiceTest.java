...
class UnidadeSaudeServiceTest {
    ...
    void deveListarUsuariosDaUnidade() {
        ...
        when(unidadeSaudeRepository.findUsuariosByUnidadeSaudeId(1L))
                .thenReturn(List.of(usuario));

        List<UsuarioResponseDTO> resultado =
                unidadeSaudeService.listarUsuariosPorUnidade(1L);

        assertEquals(1, resultado.size());
    }

    @Test
    void deveListarTodasUnidadesPaginado() {
        ...
        Page<UnidadeSaude> page =
                new PageImpl<>(List.of(unidade));

        when(unidadeSaudeRepository.findAllByOrderByNomeAsc(pageable))
                .thenReturn(page);

        Page<UnidadeSaudeResponseDTO> resultado =
                unidadeSaudeService.listarTodos(pageable);

        assertEquals(1, resultado.getTotalElements());
        assertEquals("UBS Centro",
                resultado.getContent().get(0).getNome());
    }

    @Test
    void deveListarUnidadesFiltradas() {
        ...
        when(unidadeSaudeRepository.buscarFiltradas(
                "Centro",
                pageable))
                .thenReturn(page);

        Page<UnidadeSaudeResponseDTO> resultado =
                unidadeSaudeService.listarTodosFiltrados(
                        "Centro",
                        pageable);
